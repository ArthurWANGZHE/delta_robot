...
namespace Example
{
    ...
    public partial class Cfg : Form
    {
        ...
        private void Cfg_Load(object sender, EventArgs e)
        {
            ...
            for (i = 0; i < Global.g_naxis; i++)
            {
                st = IMC_Pkg.PKG_IMC_GetConfig(Global.g_handle, ref m_cfg[i].steptime, ref m_cfg[i].pulpolar, ref m_cfg[i].dirpolar,
                    ref m_cfg[i].encpena, ref m_cfg[i].encpmode, ref m_cfg[i].encpdir, 
        ...
        public void UpdateList(int row, CFG_INFO info)
        {
            ...
            listView1.Items[row].SubItems[8].Text = info.acclim.ToString("f4");
            listView1.Items[row].SubItems[9].Text = info.vellim.ToString("f4");
            listView1.Items[row].SubItems[10].Text = info.plimitena != 0 ? ("使能") : ("禁止");
            listView1.Items[row].SubItems[11].Text = info.plimitpolar != 0 ? ("高电平") : ("低电平");
            listView1.Items[row].SubItems[12].Text = info.nlimitena != 0 ? ("使能") : ("禁止");
            listView1.Items[row].SubItems[13].Text = info.nlimitpolar != 0 ? ("高电平") : ("低电平");
            listView1.Items[row].SubItems[14].Text = info.almena != 0 ? ("使能") : ("禁止");
            listView1.Items[row].SubItems[15].Text = info.almpolar != 0 ? ("高电平") : ("低电平");
        }
        //获得修改后的信息
